...
public class JGLFontImpl implements JGLFont {
  ...
  /* (non-Javadoc)
   * @see butbzdorov.client.guiLib.utils.jglfontCore.impl.JGLFont#renderText(int, int, java.lang.String)
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see butbzdorov.client.guiLib.utils.jglfontCore.impl.JGLFont#renderText(int, int, java.lang.String, float, float, float, float, float, float)
   */
  @Override
  public void renderText(
      final int x,
    ...
    for (int offset = 0; offset < text.length(); /* no increment */) {
      ...
      if (characterInfo != null) {
        ...
        xPos += getCharacterWidth(currentCodepoint, nextCodePoint, sizeX);
      }
    }
