package butbzdorov.client.guiLib.utils.lwjgl;
...
import butbzdorov.client.guiLib.utils.lwjgl.io.ImageData;
import butbzdorov.client.guiLib.utils.lwjgl.io.ImageIOImageData;
import butbzdorov.client.guiLib.utils.lwjgl.io.TGAImageData;
...
public class LwjglBitmapFontImage {
  ...
  private void createTexture(
    ...
    // Если размеры текстуры больше максимального разрешения, возвращаем ошибку
    if (width > max || height > max) {
      log.warning("Texture dimensions are too large: " + width + "x" + height);
      return;
    }

    // Если разрешение текстуры подходит, создаем буфер
    ByteBuffer largerBuffer = BufferUtils.createByteBuffer(width * height * 4);  // Увеличиваем размер буфера (например, 4 компонента на пиксель)

    // Копируем данные в увеличенный буфер
    textureBuffer.rewind();
    largerBuffer.put(textureBuffer);
    largerBuffer.rewind();
    ...
    if (minFilter == GL11.GL_LINEAR_MIPMAP_NEAREST) {
      GLU.gluBuild2DMipmaps(GL11.GL_TEXTURE_2D, componentCount, width, height, srcPixelFormat, GL11.GL_UNSIGNED_BYTE, largerBuffer);
    } else {
      GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA8, width, height, 0, srcPixelFormat, GL11.GL_UNSIGNED_BYTE, largerBuffer);
    }
